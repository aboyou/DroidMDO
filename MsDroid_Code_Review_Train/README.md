...
آرگومان `data` یک شیء از نوع:

```python
torch_geometric.data.Data
```

یا در حالت batch‌شده:

```python
torch_geometric.data.Batch
```

است که ساختار یک **گراف** (یا مجموعه‌ای از گراف‌ها) را در PyTorch Geometric (PyG) نمایش می‌دهد.

---

## 🔍 محتوای `data` چیست؟

در PyG، آبجکت `data` معمولاً شامل این فیلدهاست:

|ویژگی (`data.attr`)|نوع داده|توضیح|
|---|---|---|
|`data.x`|`torch.Tensor [num_nodes × num_features]`|ویژگی گره‌ها|
|`data.edge_index`|`torch.LongTensor [2 × num_edges]`|لیست یال‌ها (source → target)|
|`data.edge_attr`|`torch.Tensor [num_edges × ?]` (اختیاری)|ویژگی یال‌ها (اگر باشد)|
|`data.y`|`torch.Tensor`|برچسب گراف یا گره‌ها|
|`data.batch`|`torch.LongTensor [num_nodes]`|مشخص می‌کند هر گره به کدام گراف تعلق دارد (در حالت batch‌شده)|
|`data.num_nodes`|`int`|تعداد گره‌ها|
|`data.num_node_features`|`int`|تعداد ویژگی‌های گره‌ها|

---

## 📦 در `GNNStack.forward()`

تابع `forward` این اجزای `data` را استفاده می‌کند:
...
- **`x`**: ماتریس ویژگی‌های گره‌ها
- **`edge_index`**: ساختار توپولوژی گراف
- **`batch`**: برای اعمال global pooling روی گراف‌های مختلف در یک batch

---

## ✅ مثال عینی از `data`

فرض کن یک گراف با 3 گره و 2 یال داریم:

```python
from torch_geometric.data import Data
import torch

x = torch.tensor([[1, 0], [0, 1], [1, 1]], dtype=torch.float)  # 3 گره، 2 ویژگی
edge_index = torch.tensor([[0, 1], [1, 2]], dtype=torch.long).t()  # دو یال: 0→1 و 1→2

data = Data(x=x, edge_index=edge_index)
```

اگر این گراف را به مدل بدهیم:

```python
model.forward(data)
```

مدل `x`، `edge_index` و (در صورت batch) `data.batch` را استفاده می‌کند.
...
# جریان آموزش
![trainFlow](trainFlow.png)
## 🟢 1. مرحله آماده‌سازی

|مرحله|توضیح|
|---|---|
|`get_device()`|تشخیص CPU یا GPU برای آموزش|
|`load model`|اگر مدل قبلی وجود دارد، وزن‌ها بارگذاری می‌شوند|
|`GNNStack(...)`|ساخت مدل شبکه گرافی (مثل GIN یا GAT) با تنظیمات|
|`Adam(...)`|مقداردهی اولیه به بهینه‌ساز (optimizer)|

---

## 🔁 2. حلقه آموزش بر اساس epoch

برای هر epoch:

1. **مدل به حالت training می‌رود**
2. مقدار total loss صفر می‌شود
3. تایمر شروع می‌شود
...
|گام|عملکرد|
|---|---|
|`real_batch(batch)`|تبدیل داده از حالت APK-level به API-level (باز کردن زیرگراف‌ها)|
|`model(batch)`|عبور از لایه‌های GNN و دریافت embedding و پیش‌بینی|
|`apk_loss(...)`|محاسبه loss بر اساس ساختار خاص APK (مجموعه‌ای از APIها)|
|`loss.backward()` + `opt.step()`|گرادیان‌گیری و به‌روزرسانی وزن‌ها|
|پاک‌سازی حافظه CUDA بعد از هر batch||
...
|مرحله|توضیح|
|---|---|
|میانگین‌گیری loss|مجموع کل / تعداد نمونه‌ها|
|`my_test(...)`|ارزیابی مدل روی مجموعه تست|
|محاسبه f1، f2|معیارهای ارزیابی|
|لاگ به `TensorBoard`|برای مشاهده گراف و عددهای آموزشی در ابزارهای تصویری|

---

## 💾 5. ذخیره مدل

مدل ذخیره می‌شود اگر:

- f1 یا هر معیار دیگر بهتر از رکورد قبلی شود
- یا در فاز اواخر آموزش باشد و عملکرد قابل قبول باشد

همچنین مدل با کمترین train loss هم جداگانه ذخیره می‌شود.
...
مدل آموزش‌دیده و شماره epoch برگشت داده می‌شود.